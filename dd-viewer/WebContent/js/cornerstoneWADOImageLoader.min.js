...
function throw_wado_error(imageId){
	...
	if($(series).length>1){
		...
		if($(series).find('label').length>0){
			$(series).find('img').remove();
			$(series).find('label').remove();
			$(series).append('<img src="images/xroja.png" style="position:absolute;left:45px;top:35px;width:20px;height:20px;">');
		}
		else{
			$(series).append('<img src="images/xroja.png" style="position:absolute;left:45px;top:35px;width:20px;height:20px;">');
		}
