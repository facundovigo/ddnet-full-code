...
import ddhemo.ejb.StudySearchFilter;
...
public class Study {
	...
	public Response updateReportBody(@Context HttpServletRequest request,
		...
		if (!study.isReported()) {
			...
			if (finished){
				...
				study.setUserReport(user.getLogin()+"["+user.getFirstName()+" "+user.getLastName()+"]");
				}
	...
	private void saveReportFile(final ddhemo.ejb.entities.Study study, final User editor) {
		try  {
...
/****************************************************************************************************************************************************/
	...
	private void writeStudyPDFTo(ddhemo.ejb.entities.Study study, User editor, OutputStream output) throws FOPException, IOException, TransformerException {
	...
	private String getInstitutionLogoFile(ddhemo.ejb.entities.Study study) {
		try {
	...
	private String getInstitutionFooterFile(ddhemo.ejb.entities.Study study) {
		try {
			String logosFolder = configurationManager
					.getConfigurationItem("report.logos.source-path").getValue();			
	...
	private File getStudyUploadDirectory(ddhemo.ejb.entities.Study study) {
...
/****************************************************************************************************************************************************/
	...
	private StudyDTO flatten(ddhemo.ejb.entities.Study study) {
		return flatten(Collections.singletonList(study)).iterator().next();
	}
	
	private Collection<StudyDTO> flatten(Collection<ddhemo.ejb.entities.Study> studies) {
		Collection<StudyDTO> result = new ArrayList<StudyDTO>();
		File uploadsRoot = getStudyUploadedFilesRootDirectory();
		
		StudyDTO dto = null;
		for(ddhemo.ejb.entities.Study study : studies) {
			dto = new StudyDTO();
