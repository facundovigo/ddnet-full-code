			...
			$(document).ready(function() {			
				...
				mainLayout = $('#divCuerpoPagina').layout({		
					...
					north__childOptions: {
						...
						center__childOptions: {
							west__paneSelector:		".study-search-box-west",
							center__paneSelector:	".study-search-box-center",
							east__paneSelector:		".study-search-box-east",
				    ...
				    $.getJSON('rest/studies?' + $(this).serialize())
						  .done(function(result) {
							  	...
							  	if (result && result.length > 0){
									...
									document.getElementById("btnIncidence").innerHTML = aux + " INCIDENCIA(S)";
									...
									CompletePages( pageInfo, 1 );
							  		...
							  		document.getElementById("btnIncidence").innerHTML = "0 INCIDENCIA(S)";
				...
				$('#ddnet-version').text('Version 2015.09.14');
				  ...
				  .done(function(result) {
				    	...
				    	if (result && result.length > 0) {
					    	$.each(result, function(index, dr) {
					    		medicoSelect.append('<option value="' + dr.name + '">' + dr.login + " (" + dr.name + ")" + '</option>');
					    		physiciansSelect.append('<option value="' + dr.login + '">' + dr.uname + ' ' + dr.ulname + '</option>');
					        });					    	
				    	}						  	
				    	medicoSelect.val('');
				  })
