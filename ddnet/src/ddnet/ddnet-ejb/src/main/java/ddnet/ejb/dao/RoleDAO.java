...
public class RoleDAO extends AbstractDAO<Long, Role> {
	...
	public Collection<Role> getNewRoles() {
		return entityManager.createQuery("SELECT r FROM Role r WHERE r.name!='Paciente' AND r.chores!='' ORDER BY r.name ASC", Role.class)
				.getResultList();
	}
}