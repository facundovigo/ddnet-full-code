...
function getStudyImagesFor(study, quality, seriesID) {
	...
	if (ddUserAgent.isUserAgentActive()) {
		...
		if(userinfo.prop != null){
			if (userinfo.prop.callingAet)
				callingAET = userinfo.prop.callingAet;
			if (userinfo.prop.hostname)
				viewerHostname = userinfo.prop.hostname;
			if (userinfo.prop.port)
				viewerPort = parseInt(userinfo.prop.port);
			if (userinfo.prop.aet)
				viewerAET = userinfo.prop.aet;
		}
