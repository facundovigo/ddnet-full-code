...
public class UserDAO extends AbstractDAO<Long, User> {
	...
	public Collection<User> getAdministratorUsers(){
		...
		return entityManager.createQuery("SELECT DISTINCT(ui.user) FROM UserInstitution ui "
									+ " WHERE ui.role.name LIKE '%Administrador%'", User.class)
				.getResultList();
	}

	public Collection<User> getMedicos(){
		
		List<User> result = 
				entityManager.createQuery(" SELECT DISTINCT(ui.user) FROM UserInstitution ui "
										+ "	WHERE ui.role.name LIKE '%Medico%'", User.class)
				.getResultList();
